...
LRESULT CSinstar3Impl::OnSvrNotify(UINT uMsg, WPARAM wp, LPARAM lp)
{
	...
	if (wp == NT_COMPINFO)
	{
		...
		SStringT strHotKeyFile = SStringT().Format(_T("%s\\server\\hotkey_%s.txt"), CDataCenter::getSingletonPtr()->GetDataPath().c_str(), myData.m_compInfo.strCompName);
		//加载特定的自定义状态及语句输入状态开关
